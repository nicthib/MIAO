...
function [m,data] = LoadData_v17(varargin)
%           UPDATES
...
if ~isempty(regexp(h.m.outputs,'[n]'))
    ...
    elseif isfield(data,'lime')
        ...
        data.rcamp = (data.lime-h.m.bkg)./((abs(data.red-h.m.bkg).^h.m.Dr).*(abs(data.green).^h.m.Dg));
        if ~isfield(h.m,'bgGG')
            h.m.bgGG = mean(data.rcamp(:,:,h.m.baseline),3);
        end
        data.rcamp = data.rcamp./repmat(h.m.bgGG,[1 1 size(data.rcamp,3)])-1;
    end
end
