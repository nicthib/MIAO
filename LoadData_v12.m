function [m,data] = LoadData_v12(varargin)
% [m,data] = LoadData(Path,Outputs,DSF,Pathlengths,[optional],[optional]) executes for 1 stim run only.
%
% Version 1.2
%
% Inputs:
% 1: Path - full path to stim run files
% 2: Outputs - string with up to 8 characters: rgbodn12.
% Red, Green, Blue, hbO, hbR, Neural. 1 and 2 are for webcams in CCD and 
% stimCCD folders, respectively. Order doesn't matter.
% 3: DSF - downsample factor (Integer). Default of 1.
% 4: Pathlengths - array of two pathlengths for GCaMP correction. Default
% of [.5 .6].
% 5: Rotation value equal to x*90 degrees ccw (so nrot of 2 will rotate 
% by 180 degrees). Default is 0.
% 6: This can either be 'noload' if you just want metadata for
% this run, or 'corr_flicker' if you want to correct the red channel
% flicker.
% 7: baseline frames: passed as a vector containing what frames
% you want to use as a baseline for corrrection. default is 30:100.
%
% If you want to pass an already created m struct, pass it as the 8th
% argument.
...
% 5-11-18 -added crop to zyla loading so that outputs are as intended.
%         -added flicker correction
%         -(IN PROGRESS) add blank frame deletion from zyla data
%         -(IN PROGRESS) remove large number of inputs and condense into m.
...
elseif strcmp(h.m.camera,'zyla')
    ...
    % preallocate
    for i = 1:h.m.nLEDs
        data.(h.m.LEDs{i}) = (zeros(h.m.height/h.m.dsfactor,h.m.width/h.m.dsfactor,ceil(numFramesPerSpool.*length(filesToLoad)/h.m.nLEDs)));
    end
    ...
    for j = 1:length(filesToLoad)
        ...
        for k = 1:h.m.nLEDs
            if h.m.dsfactor > 1
                data.(h.m.LEDs{k})(:,:,dsindex.(h.m.LEDs{k}){j}) = squeeze(mean(mean(reshape(rawData(1:h.m.height,1:h.m.width,indexx.(h.m.LEDs{k}){j}),[h.m.dsfactor,h.m.height/h.m.dsfactor,h.m.dsfactor,h.m.height/h.m.dsfactor,indnum.(h.m.LEDs{k}){j}]),3),1));
            else
    ...
    if strcmp(varargin{6},'corr_flicker')
...
if numel(varargin) > 1 && ~isempty(regexp(varargin{2},'[odn]'))
    [data] = convertNic(data,h.m.baseline,processing,h);
end
