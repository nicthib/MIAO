...
% Inputs:
%
% Path (REQUIRED) - full path to stim run files
%
...
% you want to use as a baseline for correction (minimum 5 frames). 
% Default is [30:100].
%
...
%
% Other optional string arguments: 
% 'noload'       : load the metadata only
% 'corr_flicker' : perform flicker correction
% 'gettruestim'  : read the stimCCD bin files and get the real stim values
% 'do_PCA'       : perform PCA noise reduction on the data before conversion
%
% Outputs:
% m: metadata struct
% data: data struct that includes WFOM and webcam image matrices.

function [m,data] = LoadData_v15(varargin)
%           UPDATES
...
if numel(varargin) == 1 && ~ischar(varargin{1})
    ...
    h.m.isgui = 0;
    noload = 0; do_PCA = 0; corr_flicker = 0; gettruestim = 0; do_parfor = 0; go_quick = 0;
    % Input parsing
    for i = 1:numel(varargin)
        ...
        elseif strcmp(tmp,'gettruestim')
            gettruestim = 1;
        elseif strcmp(lower(tmp),'do_PCA')
            do_PCA = 1;
        elseif strcmp(lower(tmp),'do_parfor')
            do_parfor = 1;
        elseif strcmp(lower(tmp),'go_quick')
            go_quick = 1;
...
elseif strcmp(h.m.camera,'zyla') && ~isempty(regexp(h.m.outputs,'[rgbodn]'))
    ...
    if ~do_parfor
        ...
        textprogressbar(sprintf('  Done'))
    elseif do_parfor
        fulldir = (h.m.fulldir); height = h.m.height; width = h.m.width;
        dsf = h.m.dsf; LEDs = h.m.LEDs; nLEDs = h.m.nLEDs;
        
        for i = 1:h.m.nLEDs  % preallocate
            red = (zeros(height/dsf,width/dsf,ceil(numFramesPerSpool.*length(filesToLoad)/nLEDs)));
            green = (zeros(height/dsf,width/dsf,ceil(numFramesPerSpool.*length(filesToLoad)/nLEDs)));
            blue = (zeros(height/dsf,width/dsf,ceil(numFramesPerSpool.*length(filesToLoad)/nLEDs)));
        end
        
        for k = 1:3;
            if strcmp(LEDs{k},'blue'); bluestart = k; end
            if strcmp(LEDs{k},'green'); greenstart = k; end
            if strcmp(LEDs{k},'red'); redstart = k; end
        end
        
        parfor i = 1:length(filesToLoad)
            FID = fopen(fullfile(fulldir,filesToLoad{i}));
            rawData = fread(FID, 'uint16=>uint16'); fclose(FID);
            numFramesInThisSpool = floor(length(rawData)/(numRows*numColumns));
            numPixelsToReshape = numRows * numColumns * numFramesInThisSpool;
            rawData = reshape(rawData,[numRows,numColumns,numFramesPerSpool]);
            rawData = rawData(1:height,1:width,:);
            fullraw(:,:,:,i) = rawData;
        end
        
        ss = size(fullraw);
        data.blue = fullraw(:,:,bluestart:nLEDs:end,:);
        data.blue = reshape(data.blue,[height width ss(3)*ss(4)/nLEDs]);
        data.red = fullraw(:,:,redstart:nLEDs:end,:);
        data.red = reshape(data.red,[height width ss(3)*ss(4)/nLEDs]);
        data.green = fullraw(:,:,greenstart:nLEDs:end,:);
        data.green = reshape(data.green,[height width ss(3)*ss(4)/nLEDs]);
    end
...
% end
if go_quick
    data.green = data.green(:,:,1:500);
    data.red = data.red(:,:,1:500);
    data.blue = data.blue(:,:,1:500);
end
